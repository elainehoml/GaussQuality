...
GaussQual_dir = os.path.join(base_dir, "GaussQual")
sys.path.append(GaussQual_dir)
...
def test_get_img_list():
    ...
    """
    img_list = GaussQual_io.get_img_list(img_dir)
...
def test_get_img_filepath():
    ...
    img_filepath = GaussQual_io.get_img_filepath(img_dir, slice_no)
...
def test_mask_img():
    ...
    # Apply mask
    masked_img = GaussQual_io.mask_img(test_img, mask_pct)

    # If mask is in centre, mean of masked_img = 1
    # If mask is correct shape, masked size / original = mask pct
    assert (np.mean(masked_img) == 1) and (masked_img.shape[0]/test_img.shape[0] == approx(mask_pct/100, rel=0.05)) and (masked_img.shape[1]/test_img.shape[1] == approx(mask_pct/100, rel=0.05))
...
def test_load_img():
    ...
    slice_no = np.random.randint(10, 50)
    img_filepath = GaussQual_io.get_img_filepath(img_dir, slice_no) 
    img_load = io.imread(img_filepath) # load with different library

    # Load image, full size
    img_test = GaussQual_io.load_img(img_filepath)
    masked_size = (int(img_test.shape[0]*0.2), int(img_test.shape[1]*0.2))

    # Load image, masked to 20%
    masked_img = GaussQual_io.load_img(img_filepath, mask_percentage=20)

    # Get a 20x20 patch of images to compare
    patch_img_load = img_load[40:60, 200:220]
    patch_img_test = img_test[40:60, 200:220]

    assert (masked_size == masked_img.shape) and (patch_img_load == approx(patch_img_test, rel=1e-1))